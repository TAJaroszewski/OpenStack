...
class StorageCenterApiHelper(object):
    ...
    def open_connection(self):
        ...
        LOG.info(_('open_connection to %(ssn)s at %(ip)s'),
                 {'ssn': ssn,
...
class StorageCenterApi(object):
    ...
    def _get_json(self, blob):
        ...
        try:
            ...
            LOG.error(_('Error invalid json: %s'),
                      blob)
        return None
    ...
    def _get_id(self, blob):
        ...
        try:
            ...
            LOG.error(_('Invalid API object: %s'),
                      blob)
        return None
    ...
    def open_connection(self):
        ...
        if r.status_code != 200:
            LOG.error(_('Login error: %(code)d %(reason)s'),
                      {'code': r.status_code,
                       'reason': r.reason})
    ...
    def close_connection(self):
        ...
        if r.status_code != 204:
            LOG.warning(_('Logout error: %(code)d %(reason)s'),
                        {'code': r.status_code,
                         'reason': r.reason})
        self.client = None
    ...
    def find_sc(self):
        ...
        result = self._get_result(r,
                                  ...
                                  self.ssn)
        if result is None:
            LOG.error(_('Failed to find %(s)s.  Result %(r)s'),
                      {'s': self.ssn,
                       'r': r})
            raise exception.VolumeBackendAPIException(
    ...
    def _create_folder_path(self, url, foldername):
        ...
        for folder in path:
            ...
            if scfolder is None:
                LOG.error(_('Unable to create folder path %s'),
                          folderpath)
                break
    ...
    def _init_volume(self, scvolume):
        ...
        LOG.warning(_('Volume initialization failure. (%s)'),
    ...
    def create_volume(self, name, size):
        ...
        if folder is None:
            LOG.warning(_('Unable to create folder %s'),
                        self.vfname)
        ...
        if r.status_code == 201:
            ...
            if scvolume:
                LOG.info(_('Created volume %(instanceId)s: %(name)s'),
                         {'instanceId': scvolume['instanceId'],
                          'name': scvolume['name']})
            else:
                LOG.error(_('ScVolume returned success with empty payload.'
            ...
            LOG.error(_('ScVolume create error '
    ...
    def delete_volume(self, name):
        ...
        LOG.warning(_('delete_volume: unable to find volume %s'),
                    name)
    ...
    def _find_serveros(self, osname='Red Hat Linux 6.x'):
        ...
        LOG.warning(_('ScServerOperatingSystem GetList return: '
                        '%(code)d %(reason)s'),
                    {'code': r.status_code,
                     'reason': r.reason})
        return None
    ...
    def create_server(self, wwnoriscsiname, isfc=False):
        ...
        r = self.client.post('StorageCenter/ScPhysicalServer',
                             payload)
        if r.status_code != 201:
            LOG.error(_('ScPhysicalServer create error: '
            ...
            if scserver is not None:
                if not self._add_hba(scserver,
                                     wwnoriscsiname,
                                     isfc):
                    LOG.error(_('Error adding HBA to server'))
    ...
    def find_server(self, instance_name):
        ...
        if hba is not None and hba.get('server') is not None:
            ...
            r = self.client.post('StorageCenter/ScServer/GetList',
                                 pf.payload)
            if r.status_code != 200:
                LOG.error(_('ScServer error: %(code)d %(reason)s'),
                          {'code': r.status_code,
                           'reason': r.reason})
            else:
                scserver = self._first_result(r)
    ...
    def _find_domains(self, cportid):
        ...
        if r.status_code == 200:
            ...
            LOG.debug('FaultDomainList error: %(code)d %(reason)s',
                      {'code': r.status_code,
                       'reason': r.reason})
            LOG.error(_('Error getting FaultDomainList'))
        return None
    ...
    def _find_fc_initiators(self, scserver):
        ...
        if r.status_code == 200:
            ...
            LOG.debug('HbaList error: %(code)d %(reason)s',
                      {'code': r.status_code,
                       'reason': r.reason})
            LOG.error(_('Unable to find FC initiators'))
        return initiators
        ...
        if scvolume.get('active', False):
            ...
            if r.status_code == 200:
                ...
                LOG.debug('MappingList error: %(code)d %(reason)s',
                          {'code': r.status_code,
                           'reason': r.reason})
                LOG.error(_('Unable to find volume mappings: %s'),
                          scvolume.get('name'))
        else:
            LOG.error(_('_find_mappings: volume is not active'))
        return mappings
    ...
    def _find_controller_port(self, cportid):
        ...
        if r.status_code == 200:
            ...
            LOG.debug('ScControllerPort error: %(code)d %(reason)s',
                      {'code': r.status_code,
                       'reason': r.reason})
            LOG.error(_('Unable to find controller port: %s'),
                      cportid)
        ...
        if len(mappings) > 0:
            ...
            for mapping in mappings:
                ...
                if controllerport is not None:
                    ...
                    if wwn is None:
                        LOG.error(_('Find_wwns: Unable to find port wwn'))
                    ...
                    if wwn is not None and serverhba is not None:
                        ...
                        if hbaname in initiators:
                            ...
                            if lun is None:
                                ...
                                LOG.warning(_('Inconsistent Luns.'))
        else:
            LOG.error(_('Find_wwns: Volume appears unmapped'))
        LOG.debug(lun)
        LOG.debug(wwns)
        LOG.debug(itmap)
    ...
    def _find_active_controller(self, scvolume):
        ...
        if r.status_code == 200:
            ...
            LOG.debug('VolumeConfiguration error: %(code)d %(reason)s',
                      {'code': r.status_code,
                       'reason': r.reason})
            LOG.error(_('Unable to retrieve VolumeConfiguration: %s'),
                      self._get_id(scvolume))
        ...
        if serverid is not None and volumeid is not None:
            ...
            LOG.debug('MapToServer error: %(code)d %(reason)s',
                      {'code': r.status_code,
                       'reason': r.reason})
        # Error out
        LOG.error(_('Unable to map %(vol)s to %(srv)s'),
                  {'vol': scvolume['name'],
                   'srv': scserver['name']})
        return None
                    ...
                    if prosrv is not None and self._get_id(prosrv) == serverid:
                        ...
                        if (r.status_code != 200 or r.ok is False):
                            LOG.debug('ScMappingProfile error: '
                                      ...
                                      {'code': r.status_code,
                                       'reason': r.reason})
                            LOG.error(_('Unable to unmap Volume %s'),
                                      volumeid)
    ...
    def create_replay(self, scvolume, replayid, expire):
        ...
        if scvolume is not None:
            ...
            r = self.client.post('StorageCenter/ScVolume/%s/CreateReplay'
                                 ...
                                 payload)
            if r.status_code != 200:
                LOG.error(_('CreateReplay error: %(code)d %(reason)s'),
                          {'code': r.status_code,
                           'reason': r.reason})
            else:
        ...
        # Quick double check.
        if replay is None:
            LOG.warning(_('Unable to create snapshot %s'),
                        replayid)
    ...
    def find_replay(self, scvolume, replayid):
        ...
        try:
            ...
            LOG.error(_('Invalid ReplayList return: %s'),
                      r)
        # If we are here then we didn't find the replay so warn and leave.
        LOG.warning(_('Unable to find snapshot %s'),
                    replayid)
    ...
    def delete_replay(self, scvolume, replayid):
        ...
        if replay is not None:
            r = self.client.post('StorageCenter/ScReplay/%s/Expire'
                                 ...
                                 {})
            if r.status_code != 204:
                LOG.error(_('ScReplay Expire error: %(code)d %(reason)s'),
                          {'code': r.status_code,
                           'reason': r.reason})
                return False
    ...
    def create_view_volume(self, volname, screplay):
        ...
        if r.status_code == 200:
            ...
            LOG.error(_('ScReplay CreateView error: %(code)d %(reason)s'),
                      {'code': r.status_code,
                       'reason': r.reason})

        if volume is None:
            LOG.error(_('Unable to create volume %s from replay'),
                      volname)
    ...
    def create_cloned_volume(self, volumename, scvolume):
        ...
        if replay is not None:
            ...
            LOG.error(_('Error: unable to snap replay'))
        return clone
    ...
    def expand_volume(self, scvolume, newsize):
        ...
        if r.status_code == 200:
            ...
            LOG.error(_('Error expanding volume '
                          '%(name)s: %(code)d %(reason)s'),
                      {'name': scvolume['name'],
                       'code': r.status_code,
                       'reason': r.reason})
    ...
    def rename_volume(self, scvolume, name):
        ...
        r = self.client.post('StorageCenter/ScVolume/%s/Modify'
                             ...
                             payload)
        if r.status_code != 200:
            LOG.error(_('Error renaming volume '
